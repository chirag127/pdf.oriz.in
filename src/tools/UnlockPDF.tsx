...
export default function UnlockPDF() {
	...
	const handleProcess = async () => {
		if (!file || !password) { setError("Please enter the PDF password."); return; }
		...
		try {
			...
			setError("Failed to unlock PDF. Please check your password.");
	...
	return (
		<div className="space-y-6">
			{!file ? (
				<FileDropzone accept={[".pdf"]} onFiles={handleFiles} title="Drop your locked PDF here" subtitle="or click to browse" />
			) : (
				<>
					...
					<div>
						...
						<input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Enter the password" className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm" />
					</div>
					...
					<div className="flex flex-wrap items-center gap-3">
						{!result ? (
							<button type="button" onClick={handleProcess} disabled={processing || !password} className="inline-flex items-center gap-2 px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg hover:bg-blue-700 disabled:bg-gray-300 disabled:cursor-not-allowed transition-all text-sm">{processing ? "Unlocking..." : "Unlock PDF"}</button>
						) : (
							<DownloadButton filename="unlocked.pdf" onClick={() => result && downloadBytes(result, "unlocked.pdf")} label="Download" />
						)}
					</div>
				</>
			)}
